  ...
  return (
    <aside className={asideClassName ?? "w-64 shrink-0 border-l border-zinc-200 bg-zinc-50 flex flex-col overflow-hidden"}>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-3 space-y-3">
        ...
        {/* Custom chord — at the top for easy access */}
        <div>
          <p className="text-xs font-medium text-zinc-400 mb-1">Custom</p>
          <form onSubmit={handleCustomSubmit} className="flex gap-1.5">
            <input
              value={customChord}
              onChange={(e) => setCustomChord(e.target.value)}
              placeholder="e.g. D/F#"
              className="flex-1 text-sm border border-zinc-200 rounded px-2.5 py-1.5 outline-none focus:border-indigo-400 bg-white"
            />
            <button
              type="submit"
        ...
        {CHORD_GROUPS.map((group) => (
          <div key={group.label}>
            <p className="text-xs font-medium text-zinc-400 mb-1">{group.label}</p>
